...
It only reads source/cereal/Param-file state and writes research evidence files.
"""
...
def main() -> int:
  ...
  if source_head != args.expected_head or source_branch != EXPECTED_BRANCH:
    print(json.dumps({
      "status": "HOLD", "reason": "source_identity_mismatch",
      "sourceHead": source_head, "expectedHead": args.expected_head,
      "sourceBranch": source_branch, "expectedBranch": EXPECTED_BRANCH,
    }, indent=2))
    return 3
  ...
  if not marker_ok or not auxiliary_off:
    print(json.dumps({
      ...
      "observerExpected": expected_observer, "observerMarker": OBSERVER_MARKER.exists(),
      "telemetryMarker": TELEMETRY_MARKER.exists(), "shadowMarker": SHADOW_MARKER.exists(),
    }, indent=2))
    return 3
  ...
  with jsonl_path.open("w", encoding="utf-8", buffering=1) as out:
    while time.monotonic() < deadline:
      ...
      event = {
        ...
        "UsbGpuActive": usbgpu_now, "guardOk": guard_ok,
      }
      out.write(json.dumps(event, separators=(",", ":"), allow_nan=False) + "\n")
      if not guard_ok:
        guard_violations += 1; break

  observer_fresh: bool | None = None; observer_write_errors = observer_fallbacks = 0
  ...
  summary = {
    "leg": args.leg, "sourceHead": source_head, "sourceBranch": source_branch,
    "samples": len(latencies), "p50ModelExecutionMs": percentile(latencies, 0.50),
    "p95ModelExecutionMs": percentile(latencies, 0.95), "p99ModelExecutionMs": percentile(latencies, 0.99),
    "maxModelExecutionMs": max(latencies) if latencies else 0.0, "frameGapCount": frame_gaps,
    "fallbackCount": fallbacks, "guardViolationCount": guard_violations,
    "observerWriteErrors": observer_write_errors, "observerStateFresh": observer_fresh,
  }
  summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
  print(json.dumps(summary, indent=2, ensure_ascii=False))
  if guard_violations: return 3
  if expected_observer and observer_fresh is not True: return 2
  return 0
