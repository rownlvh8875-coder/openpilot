...
or vehicle controls. It records active model timing and fresh hardware telemetry.
"""
...
def main() -> int:
  ...
  if not marker_ok or not isolation_ok:
    print(json.dumps({
      ...
      "telemetryExpected": expected_telemetry,
  ...
  hardware_durations: list[float] = []
  last_frame: int | None = None
  last_hw_timestamp: float | None = None
  frame_gaps = 0
  fallbacks = 0
  guard_violations = 0
  hardware_samples = 0
  hardware_valid_samples = 0
  hardware_error_samples = 0
  supply_fault_samples = 0
  max_temp_c: float | None = None
  max_memory_temp_c: float | None = None
  max_power_draw_w: float | None = None
  ...
  with jsonl_path.open("w", encoding="utf-8", buffering=1) as out:
    while time.monotonic() < deadline:
      ...
      if expected_telemetry:
        ...
        if hw is not None:
          ...
          if timestamp is not None and timestamp >= start_mono and timestamp != last_hw_timestamp:
            last_hw_timestamp = timestamp
            hardware_samples += 1
            if bool(hw.get("valid", False)):
              hardware_valid_samples += 1
            errors = hw.get("errors")
            if isinstance(errors, list) and errors:
              hardware_error_samples += 1
            if bool(hw.get("supplyFault", False)):
              supply_fault_samples += 1
            duration = finite(hw.get("sampleDurationMs"))
            if duration is not None and duration >= 0:
              hardware_durations.append(duration)
            temp = finite(hw.get("tempC"))
            mem_temp = finite(hw.get("memoryTempC"))
            power = finite(hw.get("powerDrawW"))
            if temp is not None:
              max_temp_c = temp if max_temp_c is None else max(max_temp_c, temp)
            if mem_temp is not None:
              max_memory_temp_c = mem_temp if max_memory_temp_c is None else max(max_memory_temp_c, mem_temp)
            if power is not None:
              max_power_draw_w = power if max_power_draw_w is None else max(max_power_draw_w, power)
            hw_event = {
              "timestampMonoS": timestamp,
              "valid": bool(hw.get("valid", False)),
              "errors": errors if isinstance(errors, list) else [],
              "supplyFault": bool(hw.get("supplyFault", False)),
              "sampleDurationMs": duration,
              "tempC": temp,
              "memoryTempC": mem_temp,
              "powerDrawW": power,
              "powerLimitW": finite(hw.get("powerLimitW")),
              "gpuUsagePercent": finite(hw.get("gpuUsagePercent")),
              "gpuClockMhz": finite(hw.get("gpuClockMhz")),
              "fanSpeedRpm": finite(hw.get("fanSpeedRpm")),
              "supplyVoltageMv": finite(hw.get("supplyVoltageMv")),
              "supplyCurrentMa": finite(hw.get("supplyCurrentMa")),
              "usbSpeedMbps": finite(hw.get("usbSpeedMbps")),
              "usbLinkErrorCount": finite(hw.get("usbLinkErrorCount")),
      ...
      if not guard_ok:
        guard_violations += 1
        break
      if hw_event is not None and hw_event["supplyFault"]:
        break
  ...
  summary = {
    ...
    "p50ModelExecutionMs": percentile(model_latencies, 0.50),
    "p95ModelExecutionMs": percentile(model_latencies, 0.95),
    "p99ModelExecutionMs": percentile(model_latencies, 0.99),
    ...
    "telemetryStateFresh": telemetry_fresh,
    "hardwareSamples": hardware_samples,
    "hardwareValidSamples": hardware_valid_samples,
    "hardwareErrorSamples": hardware_error_samples,
    "supplyFaultSamples": supply_fault_samples,
    "p95HardwareSampleDurationMs": percentile(hardware_durations, 0.95),
    "maxHardwareSampleDurationMs": max(hardware_durations) if hardware_durations else 0.0,
    "maxGpuTempC": max_temp_c,
    "maxMemoryTempC": max_memory_temp_c,
    "maxPowerDrawW": max_power_draw_w,
  }
  summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False, allow_nan=False) + "\n", encoding="utf-8")
  print(json.dumps(summary, indent=2, ensure_ascii=False, allow_nan=False))

  if guard_violations or supply_fault_samples:
    return 3
  if expected_telemetry and not telemetry_fresh:
    return 2
  return 0
