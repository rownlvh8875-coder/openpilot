...
export function attachRadarReview(video) {
  ...
  plots.innerHTML = `<div class="radar-plot-legend"><span style="color:#f68e37">L1 거리 (레이더)</span><span style="color:#f5d348">L2 거리</span><span style="color:#4891ff">비전 거리 / 비전 L1</span><span style="color:#f75ea0">SCC 거리·가속도</span><span style="color:#3ecd82">L1 속도·Carrot 목표 가속도</span></div>
    ...
    <p class="muted tiny" data-clock-status></p>
  ...
  let generation = 0, controller = null, running = false, lastTick = 0, current = 0, selectedTrack = null, videoWasPlaying = false;
  ...
  const hasAlignedVideo = () => videoAvailable() && (!payload || payload.videoAligned);
  const usesVideo = () => hasAlignedVideo() && current < video.duration;
    ...
    if(key!==graphCacheKey){
      ...
      function series(name,y,width=1.7){
        for(const run of payload?.graphs?.[name]||[]){
          ...
          for(const [t,value] of run.samples){const time=graphTimes.get(t);if(!valid(time)||!valid(value)){started=false;continue;}const px=x(time),py=y(value);if(started)bg.lineTo(px,py);else{bg.moveTo(px,py);started=true;}}
          bg.stroke();
  ...
  function draw() {
    ...
    find('[data-clock-status]').textContent=payload?.videoAligned&&videoAvailable()&&end>video.duration&&current>=video.duration?'영상이 끝난 구간입니다. 레이더만 별도로 재생하며 영상은 마지막 프레임에 머뭅니다.':'';
  ...
  function seek(t){
    if(!duration()||!valid(t))return;
    const wasPlaying=running||(hasAlignedVideo()&&!video.paused);
    current=Math.max(0,Math.min(t,duration()));index=nearest(current);
    if(hasAlignedVideo()){
      videoWasPlaying=false;video.currentTime=Math.min(current,video.duration);
      if(usesVideo()){running=false;videoWasPlaying=wasPlaying;if(wasPlaying)video.play().catch(()=>setStatus('영상을 재생하지 못했습니다. 다시 재생을 눌러 주세요.'));}
      else {video.pause();running=wasPlaying;if(running)animate();}
    }
    draw();
  }
  function tick(now){
    frameHandle=0;
    if(usesVideo()&&!video.paused){current=Math.min(Math.max(0,video.currentTime),duration());index=nearest(current);if(current>=duration())video.pause();draw();}
  ...
  play.onclick=()=>{
    if(!duration())return;
    if(running||(hasAlignedVideo()&&!video.paused)){running=false;videoWasPlaying=false;video.pause();draw();return;}
    if(current>=duration())seek(0);
    if(usesVideo()){running=false;video.play().catch(()=>setStatus('영상을 재생하지 못했습니다. 다시 재생을 눌러 주세요.'));}
    else {running=true;animate();draw();}
  };
  video.addEventListener('play',()=>{videoWasPlaying=true;animate();});
  video.addEventListener('pause',()=>{if(!video.ended)videoWasPlaying=false;});
  video.addEventListener('ended',()=>{
    const continuePlayback=videoWasPlaying;videoWasPlaying=false;
    if(continuePlayback&&payload?.videoAligned&&videoAvailable()&&video.duration<duration()){
      current=Math.max(current,video.duration);index=nearest(current);running=true;animate();
    }
    draw();
  });
  for(const event of ['loadedmetadata','durationchange','timeupdate','seeked','pause','emptied'])video.addEventListener(event,()=>{if(usesVideo()){current=Math.max(0,Math.min(video.currentTime,duration()));index=nearest(current);}draw();});
  ...
  async function load(selected) {
    ...
    const signal=controller.signal;running=false;videoWasPlaying=false;frames=[];times=[];payload=null;index=0;current=0;selectedTrack=null;scrub.disabled=true;draw();
    ...
    try {
      ...
      while(token===generation){
        ...
        videoWasPlaying=hasAlignedVideo()&&!video.paused;
