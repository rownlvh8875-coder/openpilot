...
def test_export_matches_desktop_controller():
  ...
  for i, item in enumerate(payload['frames']):
    ...
    desktop_selection = exporter.finite_json(asdict(selection))
    assert item['selection'] == {key: desktop_selection[key] for key in
                                ('lead_one', 'lead_two', 'cutin_diagnostics', 'cutin_predecel_candidate')}
